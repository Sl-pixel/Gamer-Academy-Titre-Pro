                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($coachings as $coaching)
                        <tr class="transition-all duration-200 ease-in-out hover:bg-gray-100">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $coaching->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $coaching->user->name ?? 'N/A' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $coaching->coach->name ?? 'N/A' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $coaching->status ?? 'N/A' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                ...
                                <form method="POST" action="{{ route('destroyCoaching', $coaching->id) }}" class="inline"
